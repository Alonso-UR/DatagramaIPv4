...
function formatear_datagrama(hex_string) {
    ...
    for (let i = 0; i < length; i += 8) {
        ...
        result += formatted_line.padEnd(20) + "// Offset: " + (i / 2) + "\n";
    }
    return result;
}
...
function procesarFormulario(event) {
    ...
    // TTL
    const ttl = parse_to_int(document.getElementById('ttl').value, document.getElementById('ttl_base').value);
    if (ttl > 255) {
    ...
    // Opciones
    const options = document.getElementById('options').value.toUpperCase().replace(/ /g, '');
    const max_options_bytes = (ihl * 4) - 20;
    const max_options_chars = max_options_bytes * 2;
    
    // Solo validar longitud si hay opciones escritas
...
Bytes 16-19: IP Destino = ${datagrama_hex.substr(32, 8)} (${dst_ip})
        ...
        if (options && options !== '') {
            resultadoHTML += `Bytes 20-${19+max_options_bytes}: Opciones = ${options}\n`;
        }
...
function updateOptionsLimit() {
    ...
    if (ihl > 5) {
        ...
        if (optionsLimit) {
            optionsLimit.innerHTML = `<span class="options-info">IHL=${ihl} → Espacio para opciones: ${extraBytes} bytes (${maxChars} caracteres hex)</span>`;
        }
    } else {
